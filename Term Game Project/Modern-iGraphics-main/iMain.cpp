...
float accelerationX = 5; // Rate at which speed changes
...
void drawLevelCompletedScreen() {
    ...
    iSetColor(255, 255, 255); // White text
    iText(SCREEN_WIDTH / 2 - 150, SCREEN_HEIGHT / 2 + 100, "LEVEL COMPLETED!", GLUT_BITMAP_TIMES_ROMAN_24);
    ...
    iSetColor(255, 255, 255);
    iText(SCREEN_WIDTH / 2 - 70, SCREEN_HEIGHT / 2 - 25, "NEXT LEVEL", GLUT_BITMAP_HELVETICA_18);
    ...
    iSetColor(255, 255, 255);
    iText(SCREEN_WIDTH / 2 - 70, SCREEN_HEIGHT / 2 - 125, "MAIN MENU", GLUT_BITMAP_HELVETICA_18);
}
void initializeLevel1() {
    ...
    for (int i = 0; i < 10; i++) {
        if (numActualTiles < MAX_LEVEL_TILES) {
            levelTiles[numActualTiles].id = ID_BRICK; // Base ID

            // Set the bottom two blocks of the right wall to FINISH_POINT (brown)
            if (i == 0 || i == 1) {
                levelTiles[numActualTiles].id = FINISH_POINT;
            }
            // NEW: Set the next two blocks (above the brown ones) to RED_FINISH_POINT
            else if (i == 2 || i == 3) {
            ...
            levelTiles[numActualTiles].width = BASE_TILE_WIDTH;
            levelTiles[numActualTiles].height = BASE_TILE_HEIGHT;
            levelTiles[numActualTiles].isSolid = true; // IMPORTANT: They must be solid for collision
...
void initializeLevel2(){
    ...
    // The outer loop controls the height of the wall (Y position)
    for (int j = 0; j < ROWS + 1; j++) {
        // The inner loop creates the 2-tile thickness (X position)
        for (int i = 0; i < 2; i++) {
            if (numActualTiles < MAX_LEVEL_TILES) {
                levelTiles[numActualTiles].id = ID_BRICK;
                // This places a tile at rightEdgeX, and another one next to it
...
void initializelevel3(){
    ...
    ballX = 50*BASE_TILE_WIDTH; // A safe distance from the 1-tile thick wall
    ballY = BASE_TILE_HEIGHT*17; // Just above the floor
    speedX = 0;
    vy = 0;
    NUM_FLOOR_TILES = 110;
    // Create the 100 base floor tiles
    ...
    for (int i = 0; i < NUM_FLOOR_TILES; i++) {
        if (numActualTiles < MAX_LEVEL_TILES) {
            levelTiles[numActualTiles].id = ID_BRICK;
            levelTiles[numActualTiles].x = LEVEL_START_X + i * BASE_TILE_WIDTH;
            levelTiles[numActualTiles].y = BASE_TILE_HEIGHT*2;
            levelTiles[numActualTiles].width = BASE_TILE_WIDTH;
            levelTiles[numActualTiles].height = BASE_TILE_HEIGHT;
            levelTiles[numActualTiles].isSolid = true;
            numActualTiles++;
    ...
    for (int i = 0; i < NUM_FLOOR_TILES; i++) {
        if (numActualTiles < MAX_LEVEL_TILES) {
            levelTiles[numActualTiles].id = ID_BRICK;
            levelTiles[numActualTiles].x = LEVEL_START_X + i * BASE_TILE_WIDTH;
            levelTiles[numActualTiles].y = BASE_TILE_HEIGHT*19;
            levelTiles[numActualTiles].width = BASE_TILE_WIDTH;
            levelTiles[numActualTiles].height = BASE_TILE_HEIGHT;
            levelTiles[numActualTiles].isSolid = true;
            numActualTiles++;
    ...
    for (int i = 6; i <=7; i++) {
        if (numActualTiles < MAX_LEVEL_TILES) {
            levelTiles[numActualTiles].id = ID_BRICK;
            levelTiles[numActualTiles].x = LEVEL_START_X + i * BASE_TILE_WIDTH;
            levelTiles[numActualTiles].y = BASE_TILE_HEIGHT*7;
            levelTiles[numActualTiles].width = BASE_TILE_WIDTH;
            levelTiles[numActualTiles].height = BASE_TILE_HEIGHT;
            levelTiles[numActualTiles].isSolid = true;
            numActualTiles++;
    ...
    for (int i = 2; i <=4; i++) {
        if (numActualTiles < MAX_LEVEL_TILES) {
            levelTiles[numActualTiles].id = ID_BRICK;
            levelTiles[numActualTiles].x = LEVEL_START_X + i * BASE_TILE_WIDTH;
            levelTiles[numActualTiles].y = BASE_TILE_HEIGHT*11;
            levelTiles[numActualTiles].width = BASE_TILE_WIDTH;
            levelTiles[numActualTiles].height = BASE_TILE_HEIGHT;
            levelTiles[numActualTiles].isSolid = true;
            numActualTiles++;
    ...
    for (int i = 10; i <= 13; i++) {
        for (int j = 2; j <=5; j++)
            if((i==10 || i==11) ||(i==12 && (j==4 || j==3)) || (i==13 && j==3))
            {
                levelTiles[numActualTiles].id = ID_BRICK;
                levelTiles[numActualTiles].x = LEVEL_START_X + i * BASE_TILE_WIDTH;
                levelTiles[numActualTiles].y = LEVEL_START_Y + j * BASE_TILE_HEIGHT;
    ...
    levelTiles[numActualTiles].y = LEVEL_START_Y + 4 * BASE_TILE_HEIGHT;
    levelTiles[numActualTiles].width = BASE_TILE_WIDTH;
    levelTiles[numActualTiles].height = BASE_TILE_HEIGHT;
    ...
    levelTiles[numActualTiles].y = LEVEL_START_Y + 3 * BASE_TILE_HEIGHT;
    levelTiles[numActualTiles].width = BASE_TILE_WIDTH;
    levelTiles[numActualTiles].height = BASE_TILE_HEIGHT;
    levelTiles[numActualTiles].isSolid = true;  // Slope should still block the ball
    numActualTiles++;
    ...
    for (int i = 20; i <= 25; i++) {
        for (int j = 2; j <=5; j++)
            if((i==22 || i==23 || i==24 || i==25) ||(i==21 && (j==4 || j==3)) || (i==20 && j==3))
            {
                levelTiles[numActualTiles].id = ID_BRICK;
                levelTiles[numActualTiles].x = LEVEL_START_X + i * BASE_TILE_WIDTH;
                levelTiles[numActualTiles].y = LEVEL_START_Y + j * BASE_TILE_HEIGHT;
    ...
    for (int i = 12; i <= 15; i++) {
        for (int j = 11; j <=14; j++)
            if((i==12 && j<=14) ||(i==13 && j<=13 )|| (i==14  && j<=12) || (i==15 && j==11))
            {
                levelTiles[numActualTiles].id = ID_BRICK;
                levelTiles[numActualTiles].x = LEVEL_START_X + i * BASE_TILE_WIDTH;
                levelTiles[numActualTiles].y = LEVEL_START_Y + j * BASE_TILE_HEIGHT;
    ...
    levelTiles[numActualTiles].id = SLOPE_LEFT;
    levelTiles[numActualTiles].x = LEVEL_START_X + 12 * BASE_TILE_WIDTH;
    levelTiles[numActualTiles].y = LEVEL_START_Y + 15 * BASE_TILE_HEIGHT;
    levelTiles[numActualTiles].width = BASE_TILE_WIDTH;
    levelTiles[numActualTiles].height = BASE_TILE_HEIGHT;
    levelTiles[numActualTiles].isSolid = true;  // Slope should still block the ball
    numActualTiles++;
    ...
    levelTiles[numActualTiles].id = SLOPE_LEFT;
    levelTiles[numActualTiles].x = LEVEL_START_X + 13 * BASE_TILE_WIDTH;
    levelTiles[numActualTiles].y = LEVEL_START_Y + 14 * BASE_TILE_HEIGHT;
    levelTiles[numActualTiles].width = BASE_TILE_WIDTH;
    levelTiles[numActualTiles].height = BASE_TILE_HEIGHT;
    levelTiles[numActualTiles].isSolid = true;  // Slope should still block the ball
    numActualTiles++;
    ...
    levelTiles[numActualTiles].id = SLOPE_LEFT;
    levelTiles[numActualTiles].x = LEVEL_START_X + 14 * BASE_TILE_WIDTH;
    levelTiles[numActualTiles].y = LEVEL_START_Y + 13 * BASE_TILE_HEIGHT;
    levelTiles[numActualTiles].width = BASE_TILE_WIDTH;
    levelTiles[numActualTiles].height = BASE_TILE_HEIGHT;
    levelTiles[numActualTiles].isSolid = true;  // Slope should still block the ball
    numActualTiles++;
    ...
    levelTiles[numActualTiles].y = LEVEL_START_Y + 12 * BASE_TILE_HEIGHT;
    levelTiles[numActualTiles].width = BASE_TILE_WIDTH;
    levelTiles[numActualTiles].height = BASE_TILE_HEIGHT;
    levelTiles[numActualTiles].isSolid = true;  // Slope should still block the ball
    numActualTiles++;
    ...
    for (int i = 18; i <= 23; i++) {
        for (int j = 11; j <=15; j++)
            if(((i==22 || i==23)||((i==21 && j<=14) ||(i==20 && j<=13 )|| (i==19  && j<=12) || (i==18 && j==11))))
            {
                levelTiles[numActualTiles].id = ID_BRICK;
                levelTiles[numActualTiles].x = LEVEL_START_X + i * BASE_TILE_WIDTH;
                levelTiles[numActualTiles].y = LEVEL_START_Y + j * BASE_TILE_HEIGHT;
    ...
    for (int i = 20; i < NUM_FLOOR_TILES; i++) {
        if (numActualTiles < MAX_LEVEL_TILES) {
            levelTiles[numActualTiles].id = ID_BRICK;
            levelTiles[numActualTiles].x = LEVEL_START_X + i * BASE_TILE_WIDTH;
            levelTiles[numActualTiles].y = BASE_TILE_HEIGHT*11;
    ...
    levelTiles[numActualTiles].x = LEVEL_START_X + 43 * BASE_TILE_WIDTH;
    levelTiles[numActualTiles].y = LEVEL_START_Y + 18 * BASE_TILE_HEIGHT;
    levelTiles[numActualTiles].width = BASE_TILE_WIDTH;
    levelTiles[numActualTiles].height = BASE_TILE_HEIGHT;
    levelTiles[numActualTiles].isSolid = true;  // Slope should still block the ball
    numActualTiles++;
    ...
    levelTiles[numActualTiles].id = SLOPE_RIGHT_FLIPPED;
    levelTiles[numActualTiles].x = LEVEL_START_X + 44 * BASE_TILE_WIDTH;
    levelTiles[numActualTiles].y = LEVEL_START_Y + 17 * BASE_TILE_HEIGHT;
    levelTiles[numActualTiles].width = BASE_TILE_WIDTH;
    levelTiles[numActualTiles].height = BASE_TILE_HEIGHT;
    levelTiles[numActualTiles].isSolid = true;  // Slope should still block the ball
    numActualTiles++;
    ...
    {
        for (int j = 12; j <=14; j++)
            if((i==58 && j==12) || (i==59 && (j==12 || j==13)) ||((i==60 || i==61)&& j==14) )
            {
                levelTiles[numActualTiles].id = ID_BRICK;
                levelTiles[numActualTiles].x = LEVEL_START_X + i * BASE_TILE_WIDTH;
                levelTiles[numActualTiles].y = LEVEL_START_Y + j * BASE_TILE_HEIGHT;
...
void iMouse(int button, int state, int mx, int my) {
    if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {
        ...
        // --- NEW: Handle clicks on Level Completed screen ---
            ...
            // "NEXT LEVEL" button
            float next_btn_x = SCREEN_WIDTH / 2 - MENU_BUTTON_WIDTH / 2;
            float next_btn_y = SCREEN_HEIGHT / 2 - 50;
            if (mx >= next_btn_x && mx <= next_btn_x + MENU_BUTTON_WIDTH &&
                my >= next_btn_y && my <= next_btn_y + MENU_BUTTON_HEIGHT) {

                currentLevel++; // Increment level
                // Assuming you have levels 1, 2, 3 and want to cycle or have a maximum
                if (currentLevel > 3) { // Adjust '3' if you have more levels, or go to a "Game Won" screen
                    currentLevel = 1; // Loop back to level 1 for now
                }
                loadLevel(currentLevel); // Load the next level (this will also reset ball position and rings)
                gameState = STATE_PLAYING; // Start playing the new level
                // Optionally restart music (it was stopped when level completed)
                if (musicEnabled && !musicPlaying) {
            ...
            if (mx >= menu_btn_x && mx <= menu_btn_x + MENU_BUTTON_WIDTH &&
                ...
                gameState = STATE_TITLE; // Go back to the main menu
                currentLevel = 1; // Reset to level 1 for next play
                // Reset any game-specific variables for a fresh start on next PLAY
                ballLives = 2;
                ringsCollected = 0;
                ballX = 100.0f;
                ballY = BASE_TILE_HEIGHT + 1;
...
{
    // Movement and Jump keys are only active in STATE_PLAYING
    if (gameState == STATE_PLAYING) {
        ...
        if (key == '\b') { // Backspace
            ...
            if ((key >= 'a' && key <= 'z') || (key >= 'A' && key <= 'Z') || (key >= '0' && key <= '9') || key == ' ') {
                ...
                playerNameIndex++;
                playerNameInput[playerNameIndex] = '\0'; // Null-terminate
            }
        }
    }

    // Pause/Unpause Toggle (always active if gameState is PLAYING or PAUSED)
    if (key == 'p') {
        if (gameState == STATE_PLAYING) {
            gameState = STATE_PAUSED;
        } else if (gameState == STATE_PAUSED) {
            gameState = STATE_PLAYING; // Press 'p' again to resume
        }
    }

    // Press 'm' to return to Main Menu from playing, paused, or help screens
    if (key == 'm') {
        if (gameState == STATE_PLAYING || gameState == STATE_PAUSED || gameState == STATE_HELP_ADVENTURE || gameState == STATE_LEADERBOARD || gameState == STATE_GAME_SUMMARY) { // Added STATE_GAME_SUMMARY
            gameState = STATE_TITLE; // Change to Main Menu state
            // Reset game variables for a fresh start
            ballLives = 2;
            ringsCollected = 0;
            ballX = 100.0f;
    ...
    // Note: GLUT_KEY_ESCAPE is typically handled in iSpecialKeyboard,
    // but if you prefer it here, 27 is the ASCII code for ESC.
...
void updateBall() {
    ...
    if (fabs(speedX) < 0.1) { // A slightly larger threshold to stop sooner
        speedX = 0;
    }

    // --- 2. Handle Horizontal Movement and Collision ---
    ballX += speedX; // Move the ball horizontally
    ...
    for (int i = 0; i < numActualTiles; ++i) {
        ...
        // Check for an overlap between the ball and the tile
        if (ballX + ballWidth > tile.x && ballX < tile.x + tile.width &&
            ...
            if (speedX > 0) { // Moving right
                ballX = tile.x - ballWidth; // Move ball to the left edge of the tile
            } else if (speedX < 0) { // Moving left
                ballX = tile.x + tile.width; // Move ball to the right edge of the tile
            }
    ...
    onGround = false;
    ballY += vy; // Move the ball vertically
    ...
    for (int i = 0; i < numActualTiles; ++i) {
        ...
        if (ballX + ballWidth > tile.x && ballX < tile.x + tile.width &&
            ...
            // If a collision happened, resolve it
            if (vy > 0) { // Moving up
                ballY = tile.y - ballHeight; // Move ball to the bottom edge of the tile
            } else { // Moving down
                ballY = tile.y + tile.height; // Move ball to the top edge of the tile
                onGround = true; // We have landed on something
            }
            vy = 0; // Stop vertical movement
        }
    }
    // +++ 4. Ring and Spike Collision Logic +++
    for (int i = 0; i < numActualTiles; i++) {
        // Broad-phase check (is it even a special tile?)
        // --- MODIFIED: Added FINISH_POINT to the check ---
        if (levelTiles[i].id == RING1 || levelTiles[i].id == RING2 || levelTiles[i].id == SPIKE || levelTiles[i].id == FINISH_POINT) {

            // AABB collision check between ball and the special tile
            if (ballX + ballWidth > levelTiles[i].x &&
                ballX < levelTiles[i].x + levelTiles[i].width &&
                ballY + ballHeight > levelTiles[i].y &&
                ballY < levelTiles[i].y + levelTiles[i].height) {

                // If it's a ring, collect it.
                if (levelTiles[i].id == RING1 || levelTiles[i].id == RING2) {
                    levelTiles[i].id = EMPTY; // Make the ring disappear
                    ringsCollected++;
                    printf("Ring collected! Total: %d/%d\n", ringsCollected, totalRingsInLevel);

                    if (ringsCollected == totalRingsInLevel) {
                        printf("Level Complete!\n");
                        // You can add logic here to advance to the next level
                    }
                }
                // If it's a spike, reset the player.
                else if (levelTiles[i].id == SPIKE) {
                    printf("Ouch! Hit a spike.\n");
                    ballLives--; // Decrement a life
                    if (ballLives <= 0) {
                        printf("Game Over! No lives left.\n");
                        currentScore = ringsCollected; // Score is just ringsCollected
                        gameEndedForScoreEntry = true; // Set flag as game ended
                        gameState = STATE_ENTER_NAME; // Change game state to ENTER_NAME
                        // Stop music if playing (Using PlaySoundW)
                        PlaySoundW(NULL, NULL, 0); // Stops all currently playing sounds
                        musicPlaying = false; // Update flag
                    } else {
                        // Reset ball position if lives remain
                        ballY = BASE_TILE_HEIGHT + 1;
                        // Determine initial ballX based on currentLevel
                        if (currentLevel == 1) {
                            ballX = 100.0f;
                        } else if (currentLevel == 2) {
                            ballX = 210.0f;
                        } else if (currentLevel == 3) {
                            ballX = 50 * BASE_TILE_WIDTH; // Or whatever is appropriate for level 3 start
                        }
                        speedX = 0;
                        vy = 0;
                        onGround = true;
                ...
                // --- NEW: Handle collision with FINISH_POINT ---
                else if (levelTiles[i].id == FINISH_POINT) { //
                    printf("You reached the finish line! Level Completed!\n"); //
                    levelTiles[i].hitByBall = true; // NEW: Mark this specific tile as hit
                    currentScore = ringsCollected; // Score is just ringsCollected
                    gameEndedForScoreEntry = true; // Set flag as game ended
                    gameState = STATE_ENTER_NAME; // Go to name input
                    // Stop the ball movement instantly
                    speedX = 0; //
                    vy = 0; //
                    onGround = true; // Ensure the ball doesn't fall through after hitting
                    // PlaySoundW(NULL, NULL, 0); // Stop background music (using PlaySoundW)
                    musicPlaying = false; // Update flag
                }
                // --- END NEW ---
            }
    ...
    // --- 4. World Boundary and Camera Logic (remains the same) ---
    ...
    // Reset if ball falls out of the world
    if (ballY < -100) { // Give some leeway before reset
        ballY = BASE_TILE_HEIGHT + 1;
        ballX = 250;
        vy = 0; speedX = 0; onGround = true;
    }
    ...
    int targetCameraX = (int)(ballX + ballWidth / 2.0f) - SCREEN_WIDTH / 3;
    cameraX = targetCameraX;
    if (cameraX < 0) cameraX = 0;
    if (cameraX > maxCameraX) maxCameraX = cameraX; // Corrected: ensure cameraX doesn't go beyond maxCameraX
    if (cameraX > maxCameraX) cameraX = maxCameraX; // Ensure cameraX doesn't go beyond maxCameraX
    ...
    int targetCameraY = (int)(ballY + ballHeight / 2.0f) - SCREEN_HEIGHT / 2;
...
int main(int argc, char *argv[])
{
    ...
    // initializeTileMap();
    // Example image load (optional setup before loop starts)
    // ballImg = iLoadImage("ball.png");
    // loadLevel(currentLevel);
    // loadBallImageNames();
    // // Set timer before iInitialize
    // // iSetTimer(10, updateBall);
    // iSetTimer(10, gameLoop);
    // iInitialize(1920,1080, "Test Window");
    // // The lines below usually won't run
    loadBallImageNames();
    iSetTimer(10, gameLoop);
    iInitialize(1000,563, "Test Window");
